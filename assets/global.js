...
document.addEventListener('DOMContentLoaded', () => {
  const heroBanner = document.querySelector('.shopify-section.hp-slider');
  const header = document.querySelector('.shopify-section-header-sticky header');

  if (!heroBanner || !header) return;

  // Get --header-height CSS variable from the hero banner
  const getHeaderHeight = () => {
    const raw = getComputedStyle(heroBanner).getPropertyValue('--header-height');
    return parseInt(raw) || 0;
  };

  const checkScrollPosition = () => {
    const headerHeight = getHeaderHeight();
    const bannerTop = heroBanner.getBoundingClientRect().top + window.scrollY;
    const bannerHeight = heroBanner.offsetHeight;
    const triggerPoint = bannerTop + bannerHeight - headerHeight;

    if (window.scrollY >= triggerPoint) {
      header.classList.remove('initial-section');
    } else {
      header.classList.add('initial-section');
    }
  };
...
document.addEventListener('DOMContentLoaded', () => {
  const sizeChartButton = document.querySelector('.size-chart-trigger');
  const sizeChartCloseButton = document.querySelector('.size-chart-close-button');
  const sizeChartContainer = document.querySelector('.size-chart-modal-container');
  const overlay = document.querySelector('.theme-overlay');

  const openSizeChart = () => {
    sizeChartContainer?.classList.add('active');
    overlay?.classList.add('active');
  };

  const closeSizeChart = () => {
    sizeChartContainer?.classList.remove('active');
    overlay?.classList.remove('active');
  };

  sizeChartButton?.addEventListener('click', openSizeChart);
  sizeChartCloseButton?.addEventListener('click', closeSizeChart);
});
